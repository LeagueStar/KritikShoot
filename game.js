/**
 * KritikShoot — game.js  (v5 — Gameplay Layer Overhaul)
 * ─────────────────────────────────────────────────────────────────────────────
 * v4 fixes retained (engine & input bugs).
 *
 * v5 new changes:
 * 11. Wave Budget System     — each wave has a threat budget that scales up;
 *                              enemy types have costs and unlock at set waves;
 *                              _spawnWave() replaces the old random-float logic
 * 12. Line-of-Sight (LoS)   — Utils.lineIntersectsRect() raycasts to player;
 *                              Ranged enemies and Boss hold fire when blocked
 * 13. Ranged orbit direction — orbitDirection (+1 / -1) assigned at spawn so
 *                              enemies alternate CW / CCW instead of all going CW
 * 14. Piercing bullet nerf  — laser rounds carry hitCount; destroyed after 4 hits
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
class Enemy {
  ...
  /**
   * FIX 12 — LoS check: returns true if there is a clear line from this enemy
   * to the player (no wall or crate blocks the shot).
   */
  _hasLineOfSight(player) {
    const ex = this.x, ey = this.y;
    const px = player.x, py = player.y;
    for (const w of this.game.walls) {
      if (Utils.lineIntersectsRect(ex, ey, px, py, w.x, w.y, w.w, w.h)) return false;
    }
    if (this.game.crates) {
      for (const c of this.game.crates) {
        if (Utils.lineIntersectsRect(ex, ey, px, py, c.x, c.y, c.w, c.h)) return false;
      }
    }
    return true;
  }
...
class Boss {
  ...
  /**
   * FIX 12 — LoS for Boss: returns true if boss has a clear line to player.
   * Boss radial phase is omnidirectional so it ignores LoS, but a future
   * targeted-shot phase would use this. Applied here to the "aimed" shot
   * that could be added later. For now it gates only the targeted volley
   * that fires during the REST phase (not the omnidirectional radial ring).
   */
  _hasLineOfSight(player) {
    const game = this.game;
    for (const w of game.walls) {
      if (Utils.lineIntersectsRect(this.x, this.y, player.x, player.y, w.x, w.y, w.w, w.h)) return false;
    }
    if (game.crates) {
      for (const c of game.crates) {
        if (Utils.lineIntersectsRect(this.x, this.y, player.x, player.y, c.x, c.y, c.w, c.h)) return false;
      }
    }
    return true;
  }
  ...
  _update(dt) {
    ...
    if (this.enemies.length === 0 && !this.boss && this.kills >= this.wave) {
      ...
      if (this.wave % 5 === 0) {
        ...
        this._spawnWave();   // FIX 11: budget-based spawn
      }
  ...
  _spawnWalls() {
    ...
    for (let i = 0; i < wCount; i++) {
      ...
      this.walls.push({
        ...
        w, h,
        hp,
        maxHp: hp,   // FIX 3: was hardcoded 10, causing bar to always start at 30–80 %
      });
    }
